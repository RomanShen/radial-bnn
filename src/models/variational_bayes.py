...
class SVI_Base(nn.Module):
    ...
    def __init__(
        self, weight_shape, bias_shape, variational_distribution, prior, use_bias
    ):
        ...
        assert hasattr(
            distributions, variational_distribution
        ), "The variational distribution is not defined in util.distributions"
    ...
    def cross_entropy(self):
        ...
        """
        if not self.pretraining:
            weight_log_prior_mean_over_epsilon = torch.mean(
                self.prior_log_pdf(self.weight), dim=0
            )
            cross_entropy = -torch.sum(weight_log_prior_mean_over_epsilon)
            if self.use_bias:
                bias_log_prior_mean_over_epsilon = torch.mean(
                    self.prior_log_pdf(self.bias), dim=0
                )
...
class SVI_Linear(SVI_Base):
    ...
    def __init__(
        ...
        """
        super(SVI_Linear, self).__init__(
    ...
    def forward(self, x):
        ...
        if not self.pretraining:
            ...
            weight_sigma = self._rho_to_sigma(
                self.weight_rhos
            ...
            weight_epsilon = Variable(
                self.noise_distribution((train_samples,) + self.weight_mus.size())
            )
            if self.use_bias:
                # bias_epsilon [training_samples, out_features]
                bias_epsilon = Variable(
                    self.noise_distribution((train_samples,) + self.bias_mus.size())
                )
...
class _SVIConvNd(SVI_Base):
    ...
    def __init__(
        ...
        super(_SVIConvNd, self).__init__(
            weight_shape, bias_shape, variational_distribution, prior, use_bias
        )
    ...
    def forward(self, x):
        ...
        if not self.pretraining:
            ...
            if self.use_bias:
                ...
                self.bias = torch.addcmul(
                    self.bias_mus, bias_sigma, bias_epsilon
                )  # [samples, out_channels]
        else:
        ...
        x = x.unfold(
            4, self.kernel_size[1], self.stride[1]
        )  # Over H giving [N, samples, in_channels, H_fields, W_fields, H_kernel, W_kernel]
...
class SVIMaxPool2D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unfold(
            4, self.kernel_size[1], self.stride[1]
        )  #  Now this is [examples, samples, channels, pooled_H, pooled_W, size_H, size_W)
        x = x.max(6)[0].max(5)[0]
        return x
