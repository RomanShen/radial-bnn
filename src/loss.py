...
class Binary_cross_entropy:
    def compute_loss(self, y_input, y_target):
        return F.binary_cross_entropy_with_logits(
            y_input, y_target.type(torch.cuda.FloatTensor)
        )
...
class Elbo:
    ...
    def compute_loss(self, y_predicted, y_target):
        ...
        if self.regression:
            ...
            nll_tensor = (y_target - y_predicted.squeeze(dim=2).mean(dim=1)) ** 2 / (
                2 * variance
            )
        else:
            if self.binary:
                nll_tensor = F.binary_cross_entropy_with_logits(
                    y_predicted.squeeze(dim=2),
                    y_target.type(torch.cuda.FloatTensor),
                    reduction="none",
                )
            else:
                nll_tensor = F.nll_loss(
                    y_predicted.permute(0, 2, 1), y_target, reduction="none"
                )
        ...
        if self.writer is not None:
            self.writer.add_scalar(
                "cross-entropy", cross_entropy_sum / self.num_batches
            )
            self.writer.add_scalar("entropy", entropy_sum / self.num_batches)
            self.writer.add_scalar("nll_loss", nll_sum)
        kl_divergence_estimated_over_batch = (
            cross_entropy_sum - entropy_sum
        ) / self.num_batches

        return (
            nll_sum / self.batch_size,
            kl_divergence_estimated_over_batch / self.batch_size,
        )
